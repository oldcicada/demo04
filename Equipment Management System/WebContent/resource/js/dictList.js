...
function queryPage(){
	...
	$.post("view/sys/dictList",
			{"pageIndex":pageIndex,"pageSize":pageSize,"type":type,"tag":tag},
			function(data,status){
				...
				if(status){
					...
					for(var i=0;i<list.length;i++){
						var content="<tr><td>"+list[i].value
						+"</td><td>"+list[i].tag
						+"</td><td>"+list[i].type
						+"</td><td>" +list[i].description
						+"</td><td class='text-center'><a href='view/sys/dictEdit?id="+list[i].id+"'>编辑</a> "
						+"<a href='view/sys/dictDelete?id="+list[i].id+"'>删除</a>"
						+"</td></tr>";
