...
public class EmailUtil {
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, User sysUser,Long code) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "设备保修系统", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(sysUser.getEmail(), sysUser.getName(), "UTF-8"));

        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("设备保修系统密码修改", "UTF-8");
    ...
    /**
     	参考：https://blog.csdn.net/xietansheng/article/details/51673073
		1.表单输入注册时的邮箱；
		
		2.验证用户邮箱是否正确，如果用户邮箱不存在网站的用户表中，则提示用户邮箱未注册；
		
		3.发送邮件，如果用户邮箱确实存在用户表中，则组合用于验证用户信息的字符串，并构造URL发送到用户邮箱中;
		receiveMail=1044070829@qq.com & code=1523383407231
		4.记录保存到数据库
		
		5.用户登录邮箱收取邮件，点击URL链接到网站验证程序；
		http://127.0.0.1:8080/equipAdmin/validate?receiveMail=1044070829@qq.com & code=1523383407233
		
		6.网站程序通过用户请求的字符串查询本地最新未失效的记录，比对信息是否正确；
		
		7.如果正确则转到重置密码页面重新设置新密码，反之则提示用户验证无效。
    */
    	 // 收件人邮箱
        String receiveMailAccount = "1044070829@qq.com";
        User sysUser=new User();
        sysUser.setEmail(receiveMailAccount);
        sysUser.setName("张三");
        try {
        	//
			resetPassword(sysUser,System.currentTimeMillis());
		} catch (Exception e) {
			e.printStackTrace();
		}
